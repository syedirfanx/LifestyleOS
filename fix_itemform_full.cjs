...
const isRecurringLogic = `  const isRecurring = setupCategory === '💳 Recurring Expenses';`;
code = code.replace("  const isEditMode = !!initialData;", "  const isEditMode = !!initialData;\n" + isRecurringLogic);

const oldStatusSectionRegex = /<\!-- New Section: Status & Payment Method -->[\s\S]*?(?=<div className="space-y-1 pt-2">)/;
const exactHtmlMatch = code.substring(code.indexOf('{/* New Section: Status & Payment Method */}'), code.indexOf('<div className="space-y-1 pt-2">'));
...
code = code.replace(exactHtmlMatch, newSection);
fs.writeFileSync('./src/components/ItemFormModal.tsx', code);
console.log('Fixed ItemFormModal UI section');
